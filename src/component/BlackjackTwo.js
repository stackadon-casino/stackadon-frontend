...
class Blackjack extends React.Component {
  ...
  componentDidMount() {
    ...
    socket.emit('joinRoom', { roomNum: 2, firstName, id })
    ...
    return (
      <div className="blackjack">
                  ...
                  <div>
                    {cur.taken === false ? (
                      <img
                        ...
                        alt="sit here"
                        onClick={() =>
                          socket.emit('takeSeat', { ind: index, roomNum: 2 })
                        }
                      />
                      ...
                      <div className="bet">
                        ...
                        <div className="chipRow">
                          <img
                            ...
                            type="button"
                            onClick={() =>
                              socket.emit('bet', { bet: 1, index, roomNum: 2 })
                            }
                            className="chips"
                          ></img>
                          <img
                            ...
                            alt="five"
                            onClick={() =>
                              socket.emit('bet', { bet: 5, index, roomNum: 2 })
                            }
                            className="chips"
                          ></img>
                          <img
                            ...
                            alt="ten"
                            onClick={() =>
                              socket.emit('bet', { bet: 10, index, roomNum: 2})
                            }
                            className="chips"
                          ></img>
                        </div>
        ...
        <div className="columns" id="controller">
              ...
              <div>
                <button
                  ...
                  onClick={() => {
                    socket.emit('deal', 2)
                    socket.emit('deal', 2)
                    socket.emit('dealTrigger', { trigger: true, roomNum: 2 })
                    socket.emit('createOrder', { roomNum: 2, socketId: id })
                  }}
                >
                  Deal
                </button>
          ...
          <div className="buttons">
            <MyGamepad user={user} socket={socket} roomNum={2}></MyGamepad>
          </div>
        </div>
      </div>
    )
