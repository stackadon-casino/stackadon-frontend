...
class Blackjack extends React.Component {
  ...
  componentDidMount() {
    ...
    socket.emit('joinRoom', { roomNum: 6, firstName, id })
    ...
    return (
      <div className="blackjack">
                  ...
                  <div>
                    {cur.taken === false ? (
                      <img
                        ...
                        alt="sit here"
                        onClick={() =>
                          socket.emit('takeSeat', { ind: index, roomNum: 6 })
                        }
                      />
                      ...
                      <div className="bet">
                        ...
                        <div className="chipRow">
                          <img
                            ...
                            type="button"
                            onClick={() =>
                              socket.emit('bet', { bet: 5, index, roomNum: 6 })
                            }
                            className="chips"
                          ></img>
                          <img
                            ...
                            alt="ten"
                            onClick={() =>
                              socket.emit('bet', { bet: 10, index, roomNum: 6 })
                            }
                            className="chips"
                          ></img>
                          <img
                            ...
                            alt="twentyfive"
                            onClick={() =>
                              socket.emit('bet', { bet: 25, index, roomNum: 6 })
                            }
                            className="chips"
                          ></img>
                        </div>
        ...
        <div className="columns" id="controller">
              ...
              <div>
                <button
                  ...
                  onClick={() => {
                    socket.emit('deal', 6)
                    socket.emit('deal', 6)
                    socket.emit('dealTrigger', { trigger: true, roomNum: 6 })
                    socket.emit('createOrder', { roomNum: 6, socketId: id })
                  }}
                >
                  Deal
                </button>
          ...
          <div className="buttons">
            <MyGamepad user={user} socket={socket} roomNum={6}></MyGamepad>
          </div>
        </div>
      </div>
    )
